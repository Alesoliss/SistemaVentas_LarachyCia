    ...
    public class VentasEncabezadoRepository : IRepository<tbVentasEncabezado>
    {
        ...
        public RequestStatus Insert(tbVentasEncabezado item)
        {
            ...
            using (var db = new SqlConnection(Sistema_LarachContext.ConnectionString))
            {
                ...
                parameter.Add("@ClienteIdNuevo", dbType: DbType.Int32, direction: ParameterDirection.Output); // Parámetro de salida para el ClienteIdNuevo
                parameter.Add("@EmpleadoIdNuevo", dbType: DbType.Int32, direction: ParameterDirection.Output); // Parámetro de salida para el EmpleadoIdNuevo

                db.Execute(sql, parameter, commandType: CommandType.StoredProcedure);

                int venenId = parameter.Get<int>("@Venen_Id");
                int clienteIdNuevo = parameter.Get<int>("@ClienteIdNuevo");
                int empleadoIdNuevo = parameter.Get<int>("@EmpleadoIdNuevo");
        ...
        public IEnumerable<tbVentasEncabezado> ListaGeneral()
        {
            string sql = "[Venta].[SP_VentasEncabezado_Lista]";
            List<tbVentasEncabezado> result = new List<tbVentasEncabezado>();
            using (var db = new SqlConnection(Sistema_LarachContext.ConnectionString))
            {
                result = db.Query<tbVentasEncabezado>(sql, commandType: System.Data.CommandType.Text).ToList();
                return result;
            }
        }
