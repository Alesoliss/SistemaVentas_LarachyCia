...
import { Fill, Rol, RolEnviar } from 'src/app/demo/components/Models/RolViewModel';
import { NodeService } from 'src/app/demo/service/node.service';
import { ServiceService } from 'src/app/demo/api/Services/Roles.service';
...
///////////////////////////////
  ...
  updateSelectedKeys() {
    const excludedKeys = ['0', '99','100', '101', '102']; 
  ...
  this.nodeService.getFiles().then(treeFiles => {
    ...
    this.service.getDetalles(id).subscribe({
      next: (data: Fill) => {
         ...
         console.log(data[0].roles_Descripcionl)
         this.rolForm = new FormGroup({
...
confirmdeleteSelectedProducts(codigo) {
  this.deleteProductDialog = true;
  this.ID = codigo;
  console.log("El codigo es" + codigo);
}
